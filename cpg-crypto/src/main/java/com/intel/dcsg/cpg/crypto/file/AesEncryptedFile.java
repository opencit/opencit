 ...
 * 
 * How to create a new AES-encrypted file:
 * FileResource resource = new FileResource(new File("/path/to/file"));
 * AesEncryptedFile encFile = new AesEncryptedFile(resource, secretKeyBytes);
 * encFile.saveString("content to encrypt");
 * 
 * How to decrypt an existing AES-encrypted file:
 * ExistingFileResource resource = new ExistingFileResource(new File("/path/to/file"));
 * AesEncryptedFile encFile = new AesEncryptedFile(resource, secretKeyBytes);
 * String content = encFile.loadString();
 * 
 * Unlike the PasswordEncryptedFile which uses a PEM-like envelope with the header and footer,
 * the AesEncryptedFile class uses a similar envelope but without the header and footer, so it
 * looks just like the content of an HTTP or SMTP message instead of a PEM signature that might
 * be embedded somewhere. Currently this difference is only due to the different applications
 * of these two classes, but XXX TODO  in the future  they should accept an envelope object as
 * a parameter so that the organization of data can be swapped between an HTTP-like message, or
 * a PEM-like message, or something else. 
 * 
...
Encryption-Algorithm: AES128/OFB8
 ...
 * 
 * 
 * The encoded content has this structure:   base64(iv||aes(content-sha256||content))
 * 
...
//    public static final String CONTENT_SHA256 = "Content-SHA256";
    ...
    public AesEncryptedFile(Resource resource, String keyBase64) throws CryptographyException {
        this.resource = resource;
        this.secretKey = Base64.decodeBase64(keyBase64);
        this.cipher = new Aes128(secretKey);
    }
