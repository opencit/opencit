...
public class HostAgentFactory {
    ...
    public HashMap<String, ? extends IManifest> getManifest(TblHosts host, VCenterHost postProcessing) {
        try {
            ...
            ByteArrayResource resource = new ByteArrayResource(host.getSSLCertificate() == null ? new byte[0] : host.getSSLCertificate()); // XXX TODO it's the responsibility of the caller to save the TblHosts record after calling this method if the policy is trust first certificate ; we need to get tie the keystore to the database, especially for TRUST_FIRST_CERTIFICATE, so if it's the first connection we can save the certificate back to the database after connecting
...
//            String tlsPolicyName = "TRUST_FIRST_CERTIFICATE"; // txtHost.getTlsPolicy();  // XXX TODO TxtHost doesn't have this field yet
        ByteArrayResource resource = new ByteArrayResource(host.getSSLCertificate() == null ? new byte[0] : host.getSSLCertificate()); // XXX TODO we need to get tie the keystore to the database, especially for TRUST_FIRST_CERTIFICATE, so if it's the first connection we can save the certificate back to the database after connecting
