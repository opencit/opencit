...
//                log.debug("parameter size: {}", statement.getParameterMetaData().getPrecision(position)); // for example  16  when the type is CHAR(16) FOR BIT DATA
                ...
                if( parameterTypeName.equalsIgnoreCase("uuid") ) {
                    // postgresql uuid type 
                    statement.setObject(position, value.uuidValue());
                }
                else if( (parameterType == java.sql.Types.BINARY || parameterType == java.sql.Types.VARBINARY) && precision >= 16 ) {
