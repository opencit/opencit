...
main (int argc, char **argv)
{
	...
	if (result != TSS_SUCCESS) {
		printf("Failed to read certificate from NV memory. Trying again with owner password.\n"); // -jabuhacx 20120615
		...
		// replacing popup with intel password -jabuhacx 20120615
		//result = Tspi_Policy_SetSecret(hNVPolicy, TSS_SECRET_MODE_POPUP, 0, NULL); CKERR;
		result = Tspi_Policy_SetSecret(hNVPolicy, TSS_SECRET_MODE_PLAIN, strlen("intel"), "intel"); CKERR;
