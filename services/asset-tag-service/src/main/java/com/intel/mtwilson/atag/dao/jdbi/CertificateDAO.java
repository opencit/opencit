...
public interface CertificateDAO {
    @SqlUpdate("create table certificate (id bigint primary key generated always as identity, uuid char(36), certificate blob, sha256 char(64), pcrEvent char(40))")
    void create();
    
    @SqlUpdate("insert into certificate (uuid, certificate, sha256, pcrEvent) values (:uuid, :certificate, :sha256, :pcrEvent)")
    @GetGeneratedKeys
    long insert(@Bind("uuid") UUID uuid, @Bind("certificate") byte[] certificate, @Bind("sha256") String sha256, @Bind("pcrEvent") String pcrEvent);

    @SqlQuery("select id,uuid,certificate,sha256,pcrEvent from certificate where id=:id")
    Certificate findById(@Bind("id") long id);

    @SqlQuery("select id,uuid,certificate,sha256,pcrEvent from certificate where uuid=:uuid")
