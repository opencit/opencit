...
//    @Post("json:json")
    public Configuration insertConfiguration(Configuration configuration) throws SQLException {
        ...
        long configurationId = dao.insert(configuration.getUuid(), configuration.getName(), configuration.getContentType(), configuration.getContentText());
...
//            sql.addConditions(TAG.UUID.equal(query.id.toByteArray().getBytes())); // when uuid is stored in database as binary
        ...
        for(Record r : result) {
            ...
            configurations[i].setContentText(r.getValue(CONFIGURATION.CONTENT));
