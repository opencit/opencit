...
public class IdentityRequestGetChallenge implements Callable<byte[]> {
    ...
    private static byte[] returnEKBlob(byte [] key, byte [] aikDigest) {      
        try {
            ...
            */
            int cbActivation = 2 + //TPM_STRUCTURE_TAG tag = TPM_TAG_EK_BLOB
                    2 + //TPM_EK_TYPE ekType = TPM_EK_TYPE_ACTIVATE
                    4 + //UINT32 blobSize = cbActivation - (2 * sizeof(UINT16) + sizeof(UINT32))
                    2 + //TPM_STRUCTURE_TAG tag = TPM_TAG_EK_BLOB_ACTIVATE
                    4 + //TPM_ALGORITHM_ID algId = TPM_ALG_XOR
                    2 + //TPM_ENC_SCHEME encScheme = TPM_ES_NONE
                    2 + //UINT16 size
                    key.length +
                    aikDigest.length + //cbAikDigest +
                    2 + // UINT16 sizeOfSelect = 3
                    3 + // PcrSelect
                    1 + //TPM_LOCALITY_SELECTION localityAtRelease = TPM_LOC_ZERO
                    20; //TPM_COMPOSITE_HASH digestAtRelease = 0
            byte [] activationBlob = new byte[cbActivation];
            short sVal; int intVal; // parameters to contrusct the activationBlob
            int index = 0;
