...
#### <<<<< event
 ...
 echo "<measureLog>" >measureLog.xml
 #bios_xml  >>measureLog.xml
 echo "$BLANK2<txt>" >>measureLog.xml
 ...
 if [ ${#txt_measured_launch} -eq 27 -a ${#secrets_flag_set} -eq 24 ];then
   if [ $txt_status -eq 2 ];then
      echo "$BLANK2$BLANK2<txtStatus>2</txtStatus>" >>measureLog.xml
   else
      echo "$BLANK2$BLANK2<txtStatus>1</txtStatus>" >>measureLog.xml
   fi
 else
   echo "$BLANK2$BLANK2<txtStatus>0</txtStatus>" >>measureLog.xml
   echo "$BLANK2</txt>" >>measureLog.xml
   echo "</measureLog>" >>measureLog.xml
   exit 0
 fi
...
if [ $second_bit_sinit_mle_data_policy_control -ne 1 ];then
  ...
  if [ $txt_status -eq 2 -a $x403Data!="EOF" ];then
    ...
    os_sinit_data_capabilities="`get_value2 '\<os_sinit_data\>' '\<capabilities\>'`" >>measureLog.xml
    if [ "${os_sinit_data_capabilities:0:2}" == "0x" ];then
       os_sinit_data_capabilities=${os_sinit_data_capabilities:2}
    fi
  fi
fi
echo "$BLANK2$BLANK2<osSinitDataCapabilities>$os_sinit_data_capabilities</osSinitDataCapabilities>" >>measureLog.xml
echo "$BLANK2$BLANK2<sinitMleData>" >>measureLog.xml
sinit_mle_data_version="`get_value2 '\<sinit_mle_data\>' '\<version\>'`" >>measureLog.xml
echo "$BLANK2$BLANK4<version>$sinit_mle_data_version</version>" >>measureLog.xml

if [ $txt_status -eq 2 -a $x402Data!="EOF" ];then
  echo "$BLANK2$BLANK4<sinitHash>$x402Data</sinitHash>" >>measureLog.xml
else 
  sinit_mle_data_sinit_hash="`get_value '\<sinit_hash\>'`"
  echo "$BLANK2$BLANK4<sinitHash>$sinit_mle_data_sinit_hash</sinitHash>" >>measureLog.xml
fi

if [ $txt_status -eq 2 -a $x404Data!="EOF" ];then
   echo "$BLANK2$BLANK4<mleHash>$x404Data</mleHash>" >>measureLog.xml
else
  sinit_mle_data_mle_hash="`get_value '\<mle_hash\>'`"
  echo "$BLANK2$BLANK4<mleHash>$sinit_mle_data_mle_hash</mleHash>" >>measureLog.xml
fi
...
echo "$BLANK2$BLANK4<biosAcmId>$sinit_mle_data_bios_acm_id</biosAcmId>" >>measureLog.xml
...
echo "$BLANK2$BLANK4<msegValid>$sinit_mle_data_mseg_valid</msegValid>" >>measureLog.xml
...
echo "$BLANK2$BLANK4<stmHash>$sinit_mle_data_stm_hash</stmHash>" >>measureLog.xml
...
#  fi
#fi
echo "$BLANK2$BLANK4<policyControl>$sinit_mle_data_policy_control</policyControl>" >>measureLog.xml
...
echo "$BLANK2$BLANK4<lcpPolicyHash>$sinit_mle_data_lcp_policy_hash</lcpPolicyHash>" >>measureLog.xml
...
echo "$BLANK2$BLANK4<processorSCRTMStatus>$sinit_mle_data_proc_scrtm_status</processorSCRTMStatus>" >>measureLog.xml
...
echo "$BLANK2$BLANK4<edxSenterFlags>$sinit_mle_data_edx_senter_flags</edxSenterFlags>" >>measureLog.xml

#if [ $txt_status -eq 2 -a $x403Data!="EOF" ];then
#   echo "$BLANK2$BLANK4<eventSinitMleSubData>$x403Data</eventSinitMleSubData>" >>measureLog.xml
#fi

echo "$BLANK2$BLANK2</sinitMleData>" >>measureLog.xml

echo "$BLANK2$BLANK2<modules>" >>measureLog.xml
if [ $txt_status -eq 2 -a $x501Data!="EOF" ];then
  for((g=1;g<=${#x501DataArray[*]};g++));do
    xml_pcr  "${x501PcrIndexArray[$g]}" "$g" "${x501DataArray[$g]}"  "${#x501DataArray[*]}"  >>measureLog.xml
  done
else
