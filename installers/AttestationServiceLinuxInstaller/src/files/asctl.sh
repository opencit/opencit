...
configure_privacyca_user() {
  ...
  mtwilson login-password $PRIVACYCA_DOWNLOAD_USERNAME env:PRIVACYCA_DOWNLOAD_PASSWORD host_aiks:certify tpm_endorsements:create tpm_endorsements:search tpm_passwords:create tpm_passwords:retrieve tpm_passwords:search tpm_passwords:store tpms:endorse host_signing_key_certificates:create store_host_pre_registration_details:create
}
