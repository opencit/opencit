...
public class JarClassIterator implements Iterator<Class<?>> {
    ...
    public boolean hasNext() {
        ...
        while (nextClass == null && enumeration.hasMoreElements()) {
            ...
            if (accept(className)) {
                try {
                    ...
                    log.debug("Failed to load class {0} from archive {1}", nextClass, jar.getAbsolutePath(), e);
                }
            }
        }
        ...
        try {
            ...
            log.error("Cannot close archive {}", jar.getAbsolutePath(), e);
//            throw new RuntimeException(String.format("Cannot close archive %s", jar.getAbsolutePath()), e);
        }
        return false;
    }
