...
ALTER TABLE `mw_as`.`mw_mle_source` ADD COLUMN `mle_uuid_hex` CHAR(36) NOT NULL  AFTER `uuid_hex` ;
UPDATE mw_mle_source ms SET mle_uuid_hex = (SELECT m.uuid_hex FROM mw_mle m WHERE m.ID = ms.MLE_ID);

ALTER TABLE `mw_as`.`mw_pcr_manifest` ADD COLUMN `uuid_hex` CHAR(36) NOT NULL  AFTER `ID` ;
UPDATE mw_pcr_manifest SET uuid_hex = (SELECT uuid());
ALTER TABLE `mw_as`.`mw_pcr_manifest` ADD COLUMN `mle_uuid_hex` CHAR(36) NOT NULL  AFTER `uuid_hex` ;
UPDATE mw_pcr_manifest mpm SET mle_uuid_hex = (SELECT m.uuid_hex FROM mw_mle m WHERE m.ID = mpm.MLE_ID);

ALTER TABLE `mw_as`.`mw_module_manifest` ADD COLUMN `uuid_hex` CHAR(36) NOT NULL  AFTER `ID` ;
UPDATE mw_module_manifest SET uuid_hex = (SELECT uuid());
ALTER TABLE `mw_as`.`mw_module_manifest` ADD COLUMN `mle_uuid_hex` CHAR(36) NOT NULL  AFTER `uuid_hex` ;
