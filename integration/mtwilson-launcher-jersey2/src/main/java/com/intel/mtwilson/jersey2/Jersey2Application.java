 ...
 */
@ApplicationPath("/v2")
public class Jersey2Application /*extends ResourceConfig*/ extends Application {
...
//        }
        */
        ...
        log.debug("v2resourceconfig classloader is: {}", getClass().getClassLoader().getClass().getName()); // sun.misc.Launcher$AppClassLoader   explains why the extensions can't be cast... this is the system classloader and the extensions are loaded by one of the strategies in cpg-classpath.    or you might see org.eclipse.jetty.webapp.WebAppClassLoader  when using the WebAppContext handler
        if( getClass().getClassLoader().getParent() != null ) {
            log.debug("and the parent classloader is: {}", getClass().getClassLoader().getParent().getClass().getName()); // sun.misc.Launcher$ExtClassLoader      //    or you would see sun.misc.Launcher$AppClassLoader  if the current classloader is  the WebAppClassLoader
        }
        /*
