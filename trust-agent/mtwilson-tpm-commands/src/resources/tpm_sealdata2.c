 ...
 *
 * jbuhacoff: this command has been modified to allow user to specify
 * a tpm key other than the SRK to use for sealing. this means the 
 * tpm key must already be created prior to using this command - it will
 * not automatically create an rsa key. also, note that
 * the command already generates a random symmetric key (aes-256) to
 * encrypt the data and then seals that symmetric key using the tpm
 * key.  when using sealing to transfer keys, this is redundant, and
 * a separate tpm_sealkey command might be more appropriate. 
 */
...
{
	logCmdHelp(aCmd);
	logCmdOption("-k, --keyfile FILE",
		     _
		     ("Filename containing private key blob to use. Default is SRK."));
	logCmdOption("-i, --infile FILE",
		     _
		     ("Filename containing data to seal. Default is STDIN."));
	logCmdOption("-o, --outfile FILE",
		     _
		     ("Filename to write sealed data to.  Default is STDOUT."));
...
static char in_filename[PATH_MAX] = "", out_filename[PATH_MAX] = "", key_filename[PATH_MAX] = "";
...
static int parse(const int aOpt, const char *aArg)
{
	...
	case 'k':
		if (aArg) {
			strncpy(key_filename, aArg, PATH_MAX);
			rc = 0;
		}
		break;
	case 'i':
...
int main(int argc, char **argv)
{
	...
	if (genericOptHandler(argc, argv, "k:i:o:p:uz", opts,
	...
	// TSS_RESULT dataSeal(TSS_HENCDATA a_hEncdata, TSS_HKEY a_hKey, UINT32 a_len, BYTE * a_data, TSS_HPCRS a_hPcrs);	
	...
	BIO_puts(bdata, TPMSEAL_HDR_STRING); // "-----BEGIN TSS-----"

	/* Sealing key used on the TPM */
	BIO_puts(bdata, TPMSEAL_TSS_STRING); // "-----TSS KEY-----"
	...
	BIO_puts(bdata, TPMSEAL_EVP_STRING); // "-----ENC KEY-----"
	BIO_puts(bdata, TPMSEAL_KEYTYPE_SYM); // "Symmetric Key: "
	BIO_puts(bdata, TPMSEAL_CIPHER_AES256CBC); // "AES-256-CBC\n"
	bdata = BIO_push(b64, bdata);
	...
	/* Encrypted Data */
	BIO_puts(bdata, TPMSEAL_ENC_STRING);  // "-----ENC DAT-----"
	bdata = BIO_push(b64, bdata);
	...
	BIO_puts( bdata, TPMSEAL_FTR_STRING); // "-----END TSS-----"
