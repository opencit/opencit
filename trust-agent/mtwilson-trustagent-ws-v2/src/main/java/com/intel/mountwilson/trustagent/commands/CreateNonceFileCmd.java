            ...
            if( context.isQuoteWithIPAddress() ) {
                ...
                if( inetAddress instanceof Inet4Address ) {
                    ...
                    if( ((Inet6Address)inetAddress).isIPv4CompatibleAddress() ) {
                        ...
                        log.error("tpm.quote.ipv4 is enabled and requires an IPv4-compatible address but host address is IPv6: "+context.getIPAddress());                        
                    }
                }
                else {
                    log.error("tpm.quote.ipv4 is enabled but localhost address is not IPv4 compatible: {}", context.getIPAddress());
                }
                if( ipaddress == null ) {
                    throw new IllegalArgumentException("tpm.quote.ipv4 is enabled but localhost address cannot be resolved: "+context.getIPAddress());
                }
