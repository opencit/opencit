...
public class TpmCommands {
    ...
    public String tpmNvread(String index, String size, String ownerPassword, String outputFile) throws TAException, IOException {
        try {
            ...
            log.debug("running command tpm_nvread -x -t -i " + index + " -s " + size + " -pOWNER_PASSWORD -f /tmp/hash");
            String[] variables = { "OWNER_PASSWORD=" + ownerPassword };
            CommandResult cr = CommandUtil.runCommand("tpm_nvread -x -t -i " + index + " -s " + size + " -pOWNER_PASSWORD -f /tmp/hash", variables);
