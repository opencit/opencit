...
2013-12-05 09:19:14,461 DEBUG [main] c.i.d.c.a.t.Rfc822TokenTest [Rfc822TokenTest.java:64] Token value: test-userid
     ...
     */
    @Test
    public void testCreateToken() throws CryptographyException, IOException, NoSuchAlgorithmException, UnsupportedTokenVersionException, ExpiredTokenException, KeyNotFoundException {
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.809 sec
    ...
    @Test 
    public void testInvalidVersion() throws NoSuchAlgorithmException, CryptographyException, ExpiredTokenException, KeyNotFoundException {
        log.debug("Creating token");
        UUID userUuid = new UUID();
        String userId = userUuid.toString(); // with hyphens
        TokenFactory factory = new TokenFactory();
    ...
    @Test 
    public void testExpiredToken() throws NoSuchAlgorithmException, CryptographyException, UnsupportedTokenVersionException, KeyNotFoundException {
        log.debug("Creating token");
        UUID userUuid = new UUID();
        String userId = userUuid.toString(); // with hyphens
        TokenFactory factory = new TokenFactory();
